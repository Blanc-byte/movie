            ...
            @foreach ($movies as $movie)
                ...
                >
                    ...
                    <div class="p-3">
                        ...
                        <form action="{{ route('movie.addAgain', $movie->id) }}" method="POST" class="mt-6 animate-fadeIn">
                            @csrf
                            @method('DELETE')
                            <button 
                                type="submit" 
                                class="bg-green-500 text-white px-4 py-2 rounded shadow-lg hover:bg-green-600 hover:scale-105 transform transition-all ease-in-out duration-200 focus:outline-none"
